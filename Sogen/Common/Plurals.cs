		...
		static public string ToSingular(string word) {
			...
			if (_dictionary.ContainsValue(word.ToLower())) {
				foreach (KeyValuePair<string, string> kvp in _dictionary) {
					if (kvp.Value.ToLower() == word.ToLower()) return kvp.Key;
				}
			}
			...
			if (word.Length >= 4) {
				...
				switch (word.ToLower().Substring(word.Length - 4)) {
					case "bies":
					case "cies":
					case "dies":
					case "fies":
			...
			if (word.Length >= 3) {
				switch (word.ToLower().Substring(word.Length - 3)) {
					//box--boxes 
					case "ses":
			...
			if (word.Length >= 3) {
				switch (word.ToLower().Substring(word.Length - 2)) {
					case "es": {
